...
void GenerateRegisterKill(ASTList *il, Register *target) {
  if (!target) return;
  EmitILOp(il, kILOpKill, target, NULL, NULL, NULL);
}
...
Register *GenerateILForAssignmentOp(ASTList *il, ASTNode *left,
  ...
  GenerateRegisterKill(il, lvalue);
  return rvalue;
}
...
void GenerateILForCompStmt(ASTList *il, ASTNode *node) {
  ...
  for (int i = 0; i < GetSizeOfASTList(stmt_list); i++) {
    ...
    GenerateILFor(il, r, GetASTNodeAt(stmt_list, i));
    GenerateRegisterKill(il, r);
  }
}
...
void GenerateILForFuncDef(ASTList *il, ASTNode *node) {
  ...
  if (param_decl_list) {
    ...
    for (int i = 0; i < GetSizeOfASTList(param_decl_list); i++) {
      ...
      GenerateRegisterKill(il, var_regs[i]);
    }
  }
...
Register *GenerateILForExprUnaryPreOp(ASTList *il, Register *dst,
  ...
  if (IsEqualToken(op->op, "*")) {
    ...
    if (!IsBasicType(type, kTypeLValueOf)) {
      ...
      GenerateRegisterKill(il, tmp_reg);
      return dst;
    }
  ...
  if (IsEqualToken(op->op, "-")) {
    ...
    GenerateRegisterKill(il, rvalue);
    return dst;
  } else if (IsEqualToken(op->op, "~")) {
...
Register *GenerateILForExprUnaryPostOp(ASTList *il, Register *dst,
  ...
  if (IsEqualToken(op->op, "++")) {
    ...
    GenerateRegisterKill(il, inc_result);
    return dst;
  } else if (IsEqualToken(op->op, "--")) {
    ...
    GenerateRegisterKill(il, dec_result);
    return dst;
  }
  Error("Not impl");
  return NULL;
...
Register *GenerateILForExprBinOp(ASTList *il, Register *dst, ASTNode *node) {
  ...
  if (il_op_type != kILOpNop) {
    ...
    if (IsTypePointer(left_type)) {
      ...
      if (delta != 1) {
        ...
        GenerateRegisterKill(il, right);
        GenerateRegisterKill(il, delta_reg);
        EmitILOp(il, il_op_type, dst, left, effective_ofs_reg, node);
        GenerateRegisterKill(il, left);
        GenerateRegisterKill(il, effective_ofs_reg);
        return dst;
      }
    }
  ...
  if (IsEqualToken(bin_op->op, "&&")) {
    ...
    Register *tmp = AllocRegister();
    GenerateILRValueFor(il, tmp, bin_op->left);
    GenerateRegisterKill(il, tmp);
    GenerateILRValueFor(il, dst, bin_op->right);
    return dst;
  } else if (IsEqualToken(bin_op->op, ".")) {
    ASTType *left_type = GetExprTypeOfASTNode(bin_op->left);
...
Register *GenerateILForExprFuncCall(ASTList *il, Register *dst, ASTNode *node) {
  ...
  if (arg_list) {
    ...
    for (int i = 0; i < GetSizeOfASTList(arg_list); i++) {
      ...
      GenerateRegisterKill(il, param_values[i]);
    }
  }
...
void GenerateILForExprStmt(ASTList *il, ASTNode *node) {
  const ASTExprStmt *expr_stmt = ToASTExprStmt(node);
  if (!expr_stmt) Error("expr_stmt is NULL");
  if (!expr_stmt->expr) return;  // expr is opt
  Register *tmp_reg = AllocRegister();
  GenerateILRValueFor(il, tmp_reg, expr_stmt->expr);
  GenerateRegisterKill(il, tmp_reg);
}

void GenerateILForJumpStmt(ASTList *il, ASTNode *node) {
  ...
  if (IsEqualToken(jump_stmt->kw->token, "return")) {
    ...
    GenerateRegisterKill(il, return_value);
    return;
...
void GenerateILForIfStmt(ASTList *il, ASTNode *node) {
  ...
  GenerateRegisterKill(il, cond_result);

  Register *true_result = AllocRegister();
  { GenerateILFor(il, true_result, if_stmt->true_stmt); }
  GenerateRegisterKill(il, true_result);
  ...
  if (if_stmt->false_stmt) {
    ...
    Register *false_result = AllocRegister();
    { GenerateILFor(il, false_result, if_stmt->false_stmt); }
    GenerateRegisterKill(il, false_result);
  }
...
void GenerateILForWhileStmt(ASTList *il, ASTNode *node) {
  ...
  Register *body_result = AllocRegister();
  GenerateILFor(il, body_result, stmt->body_stmt);
  GenerateRegisterKill(il, body_result);
...
void GenerateILForForStmt(ASTList *il, ASTNode *node) {
  ASTForStmt *stmt = ToASTForStmt(node);

  Register *init_result = AllocRegister();
  { GenerateILFor(il, init_result, stmt->init_expr); }
  GenerateRegisterKill(il, init_result);
  ...
  Register *body_stmt_reg = AllocRegister();
  { GenerateILFor(il, body_stmt_reg, stmt->body_stmt); }
  GenerateRegisterKill(il, body_stmt_reg);

  EmitILOp(il, kILOpLabel, NULL, NULL, NULL, ToASTNode(stmt->continue_label));

  Register *updt_stmt_reg = AllocRegister();
  { GenerateILFor(il, updt_stmt_reg, stmt->updt_expr); }
  GenerateRegisterKill(il, updt_stmt_reg);
...
void GenerateILRValueFor(ASTList *il, Register *dst, ASTNode *node) {
  ...
  if (!IsBasicType(lvalue_type, kTypeLValueOf)) {
    ...
    GenerateRegisterKill(il, lvalue_reg);
    return;
  }
  ...
  if (node->type == kASTList) {
    ...
    for (int i = 0; i < GetSizeOfASTList(decl->init_decltors); i++) {
      ...
      if (decltor->initializer) {
        Register *tmp_reg = AllocRegister();
        GenerateILFor(il, tmp_reg, ToASTNode(decltor->initializer));
        GenerateRegisterKill(il, tmp_reg);
      }
    }
    return;
    ...
    GenerateILForWhileStmt(il, node);
  } else if (node->type == kASTForStmt) {
    GenerateILForForStmt(il, node);
  } else if (node->type == kASTIfStmt) {
    GenerateILForIfStmt(il, node);
  } else if (node->type == kASTCompStmt) {
    GenerateILForCompStmt(il, node);
  } else if (node->type == kASTExprStmt) {
    GenerateILForExprStmt(il, node);
  } else if (node->type == kASTJumpStmt) {
    GenerateILForJumpStmt(il, node);
